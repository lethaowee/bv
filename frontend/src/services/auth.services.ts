...
class AuthService {
    ...
    async login(data: any) {
        try {
            const tokens = (await this.api.post("http://localhost:3000/api/nguoidung/login", data));
            return tokens.data;
    ...
    async register(data: any) {
        try {
            const resp = (await this.api.post("http://localhost:3000/api/nguoidung/register", data));
            return resp.data;
        } catch (err: any) {
    ...
    async getMe(token: string) {
        return await axios.get("http://localhost:3000/api/nguoidung/me", {
          headers: {
            Token: token
          }
        }).then((res) => {
